...
function App() {
  ...
  const handleSubmit = function(event) {
    ...
    newTodos.sort((a,b) => a.isDone - b.isDone); //Sort işlemi için 1,12,3 önlemek için   harflere göre değil index' göre sıralama
    setTodos(newTodos);
    setTitle("");
  };

  const updateItem = function(event, index) {
    const newTodos = [...todos];//öğeleri saç ve yeni oluştur
